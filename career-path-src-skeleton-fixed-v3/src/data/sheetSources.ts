export const SHEET_SOURCES = {
  nodesCsvUrl:
    'https://docs.google.com/spreadsheets/d/e/2PACX-1vQmxVYnDbPmy4_vXfPGrnGnhn_y7CL-F2kcxOcbqc-e1Gq2oOaGH4xShCB-si0UCts2oKBGFhpyQ06_/pub?gid=29728032&single=true&output=csv',
  edgesCsvUrl:
    'https://docs.google.com/spreadsheets/d/e/2PACX-1vQmxVYnDbPmy4_vXfPGrnGnhn_y7CL-F2kcxOcbqc-e1Gq2oOaGH4xShCB-si0UCts2oKBGFhpyQ06_/pub?gid=1363019978&single=true&output=csv',
} as const;
